...
建议先看已经纳入测试的 [Focus Timer 示例插件](./examples/plugins/focus-timer/) 理解 storage 与宠物发言，再看 [Weather Status 示例插件](./examples/plugins/weather-status/) 理解 network allowlist，然后阅读完整 [插件开发文档](./docs/plugin-development.md)。
...
项目使用 **Node 原生 test runner** 覆盖 service / release / IPC / 示例插件路径，当前 **264 个测试全部通过**；同时维护 **Playwright Control Center UI 回归基线**，当前 9 个 UI 测试通过。
...
测试覆盖：
- ✅ Service / release / IPC / example 门禁覆盖（36 个测试文件）
...
- ✅ Focus Timer 与 Weather Status 示例插件通过真实本地插件服务覆盖 inspect / install / run
...
- ⚡ Focus Timer 与 Weather Status 之外的更多示例插件（RSS）
- ⚡ 插件开发教程视频
- ⚡ 用户反馈收集与迭代
