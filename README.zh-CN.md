...
OpenPet 使用统一的第三方包模型：扩展。出于兼容性，包清单文件仍叫 `plugin.json`。宿主现在可以规范化并审查 `entries.commands`、`entries.services`、`entries.dashboards`、`manifest`、`config` 和 `assets` 声明；JavaScript 兼容包可以通过现有 runner 暴露 `entries.commands`。已启用插件可以从 Control Center 显式打开声明的 HTTP/HTTPS dashboard、启动或停止声明的 service entry，并手动检查声明的 loopback service health endpoint。Service 不会自动启动，命令启动不经过 shell 展开，health check 不会后台轮询；setup、bridge flow、通用 shell command execution 和完整进程树清理仍属于后续 runtime 工作。
