...
test('ai talk service exposes and manages memory profile without reinjecting deleted memories', async () => {
  ...
  const afterRestoreProfile = service.restoreMemory(globalMemoryId)
  await service.chat({ message: '再开始一次' })
  const afterClearProfile = service.clearPetPackMemories()
  await service.chat({ message: '第三次' })
  ...
  assert.deepEqual(afterDeleteProfile.deletedMemories.map((memory) => memory.text), ['User likes quiet morning planning.'])
  ...
  assert.deepEqual(afterRestoreProfile.globalMemories.map((memory) => memory.text), ['User likes quiet morning planning.'])
  assert.deepEqual(afterRestoreProfile.deletedMemories, [])
  assert.match(JSON.stringify(requests[1].messages), /User likes quiet morning planning/)
  assert.match(JSON.stringify(requests[1].messages), /Mochi checks in softly before focus work/)
  assert.deepEqual(afterClearProfile.petPackMemories, [])
  assert.doesNotMatch(JSON.stringify(requests[2].messages), /Mochi checks in softly before focus work/)
  assert.match(JSON.stringify(requests[2].messages), /User likes quiet morning planning/)
  ...
  assert.match(JSON.stringify(logs), /ai-talk\.memory\.restored/)
