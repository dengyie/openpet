...
test('creator studio dashboard asset exists and service script is declared', () => {
  ...
  assert.match(html, /Import Approved Pet/)
  assert.match(html, /Run Review/)
  assert.match(html, /Creator prompt/)
  assert.match(html, /Generate pet/)
...
test('creator studio service exposes run detail and logs for dashboard clients', async () => {
  ...
  try {
    ...
    assert.equal(JSON.stringify(detail).includes('OPENPET_BRIDGE_TOKEN'), false)
    assert.equal(JSON.stringify(detail).includes('OPENPET_BRIDGE_URL'), false)
    assert.equal(logs.ok, true)
    assert.deepEqual(logs.logs.map((entry) => entry.event), ['run.created'])
    assert.equal(JSON.stringify(logs).includes(dataDir), false)
    assert.equal(JSON.stringify(logs).includes('OPENPET_BRIDGE_TOKEN'), false)
    assert.equal(JSON.stringify(logs).includes('OPENPET_BRIDGE_URL'), false)
