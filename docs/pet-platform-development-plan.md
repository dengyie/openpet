...
- 尚未实现第三方插件沙箱、配置 schema 表单、插件日志持久化/筛选/导出和插件存储。
...
已落地。manifest 发现和权限白名单、官方插件命令运行、Control Center 插件启停 UI、运行日志/错误面板已完成。第三方插件 JS 执行待隔离 runtime 后再开放。
...
- 重构必须小步验证（当前已有 66 个测试覆盖全部 service 和 pet-pack/plugin 模块）。
