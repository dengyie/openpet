...
OpenPet has reached the intended desktop platform shape: Electron pet runtime, React Control Center, pet packs, AI behavior, local extension documentation with explicit `entries.setup` execution, language-neutral explicit `entries.commands` process execution, narrow bridge access for explicit commands and services including read-only action discovery and bounded action preset updates, explicit dashboard opening, explicit service start/stop controls, explicit loopback service health checks, exit-confirmed best-effort service cleanup, local HTTP/MCP, release evidence tooling, and a TypeScript boundary baseline.
...
The extension ecosystem has also crossed an important platform threshold: declaration-only local command and service entries no longer need the legacy JavaScript SDK path just to make the pet speak or react. They can now receive a short-lived bridge for bounded pet actions, read-only action discovery, and bounded action preset updates while the wider ecosystem still stays open, local-first, and honest about trust limits.
...
4. Continue third-party extension author and maintainer rehearsal with real submissions, while keeping setup execution, language-neutral command execution, command/service bridge scope including read-only action discovery and bounded action preset updates, dashboard opening support, service start/stop support, manual loopback health checks, and exit-confirmed best-effort cleanup limits explicit.
