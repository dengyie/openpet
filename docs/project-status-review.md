...
| AI | OpenAI-compatible chat, main-process secret storage, grouped model-settings UI with preset-driven drafts and discovered-model apply actions, AI Talk persona/history/memory, desktop chat, Pet Bubble Chat popup, behavior decisions, replay, redacted diagnostics, memory explainability, Control Center trace export, and recent pet activity injection | `src/main/services/ai-service.js`, `src/main/services/ai-talk-service.js`, `src/main/services/ai-talk-store.js`, `src/main/services/pet-utterance-log-service.js`, `src/main/pet-chat-window.js`, `src/main/pet-bubble-chat-window.js`, `src/main/services/behavior-orchestrator-service.js` |
...
The active product gaps are release evidence, ecosystem maturity, and selective AI Talk follow-through, not a rewrite of the platform:
