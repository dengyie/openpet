...
- Extensions: current legacy SDK examples, validation, submission bundles, author rehearsal, developer-first local extension docs, normalized `entries` declarations, `entries.commands` support through the existing JavaScript compatibility runner, Control Center declaration visibility, explicit HTTP/HTTPS dashboard opening, explicit `entries.services` start/stop with runtime status and logs, and manual loopback-only health checks. Services do not auto-start and are spawned without shell expansion; setup, generic shell command execution, process-tree cleanup, and bridge flows remain future runtime work.
...
- The extension ecosystem now has four runtime-backed slices of the target model: `entries.commands` can feed the existing JavaScript compatibility runner when a package still provides `main`, enabled plugins can explicitly open declared HTTP/HTTPS dashboards from Control Center, enabled local plugins can explicitly start/stop declared service entries with logs and runtime state, and declared loopback service health endpoints can be checked manually.
- After Phase 59, the next planning step is service setup support, process-tree cleanup, bridge integration, real evidence work, community extension rehearsal, or another high-drift service/report boundary.
...
3. Decide whether the next extension phase should implement service setup, stronger stop cleanup, bridge integration, periodic health policy, or continue evidence/report hardening.
