...
> Branch: `codex/plugin-service-force-stop-phase69`
...
- The extension ecosystem now has eleven runtime-backed slices of the target model: enabled policy-allowed local plugins can explicitly run declared setup entries, `entries.commands` can feed the existing JavaScript compatibility runner when a package still provides `main`, declaration-only local `entries.commands` can run as explicit short-lived processes with JSON stdin context, declaration-only local command runs can use short-lived bridge calls for `pet.say` / `pet.action` / `pet.event` / read-only context, enabled plugins can explicitly open declared HTTP/HTTPS dashboards from Control Center, enabled local plugins can explicitly start/stop declared service entries with logs and runtime state, declared loopback service health endpoints can be checked manually, service stops attempt best-effort process-group cleanup, service stop completion is exit-confirmed, stubborn services now trigger a bounded force-stop attempt, and command/setup/service processes avoid shell expansion.
- After Phase 69, the next planning step is real signed evidence work, community extension rehearsal, setup/command cleanup parity, or another high-drift service/report boundary.
