...
- Extensions: current legacy SDK examples, validation, submission bundles, author rehearsal, and new documentation that frames third-party packages as local extensions while clearly marking services, dashboards, setup, health, and bridge flows as future runtime work.
...
- TypeScript: shared IPC/view contracts, typed Control Center API facade, typed Control Center hook state boundaries, typed Control Center pane prop surfaces, `@ts-check` main-process Control Center adapters for service/catalog/plugin/pet pack/About/update/actions payloads, full release evidence archive / signed closure report contracts, and representative payload fixtures in `npm run typecheck`.
...
- The extension ecosystem docs now use a developer-first local extension model while marking the short-lived JavaScript SDK runner as compatibility; wider runtime support for services, dashboards, setup, and bridge flows is still future work.
- After Phase 55, the next planning step is a concrete evidence-producing phase, runtime work toward the extension model, a fresh whole-project review, or another high-drift service/report boundary.
...
3. Decide whether the next extension phase should implement runtime support for command/service/dashboard entries or continue evidence/report hardening.
