...
Declaration-only command runs for creator-tools also receive host-owned `OPENPET_DATA_DIR`, `OPENPET_CACHE_DIR`, and `OPENPET_LOG_DIR`, plus bridge routes for bounded action reads/writes and read-only package-local frame inspection through `assets:inspect`. This does not grant sprite generation, arbitrary folder reads, or raw file writes.
