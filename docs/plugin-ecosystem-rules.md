...
Declaration-only command runs for creator-tools also receive host-owned `OPENPET_DATA_DIR`, `OPENPET_CACHE_DIR`, and `OPENPET_LOG_DIR`, plus bridge routes for bounded action reads/writes, read-only package-local frame inspection through `assets:inspect`, and host-mediated package-local frame import/sprite generation through `assets:generate`. This does not grant arbitrary folder reads, raw file writes, plugin-selected output paths, or pet-pack writes.
