...
- Action trigger review now closes the host-owned condition flow: `click` can update `clickAction`; `manual` and `unbound` are acknowledged; `random`, `state`, and `event` can persist host rules with editable conditions, preview summaries, and saved-rule follow-up editing from the Actions pane.
...
- The Actions pane now lets users edit host trigger-rule conditions both before first save and after persistence, using one host-owned review card instead of hard-coded default rule conditions.

P1 work:

- Close the host-owned trigger-rule condition editing loop so `random`, `state`, and `event` proposals do not rely on hard-coded default conditions after acceptance. Completed in current branch: `ActionService` now preserves proposal conditions, the host save contract can update persisted rule conditions, the Actions pane exposes condition editors and saved-rule editing, and focused core plus Playwright regressions cover the new edit/save paths.
...
- Preserve the current command paths as automation/test entry points while improving user-facing dashboard affordances. Completed in current branch: the dashboard-first flow still runs on the same durable run/task state machine, and focused Creator Studio command regressions now execute `draft-task` / `answer-question` / `confirm-task` alongside the existing `create-run` / `run-step` / `approve-run` command coverage so automation entry paths stay locked while the user-facing wizard evolves.
