...
| Phase 76 | Plugin Repository Provenance Submission Rehearsal | P1 | 已完成 repository-provenance rehearsal | `weather-status` 现在有 clone source/ref/commit/plugin subdir -> package -> submission bundle -> maintainer approval 归档证据；该证据不声明真实外部社区采用、签名信任、发布或运行安全。 |
...
- 插件作者和维护者能完成 scaffold -> validate -> package -> submission bundle -> maintainer approval record -> existing-plugin real-world rehearsal -> repository provenance rehearsal 的完整路径。
