...
| 扩展作者体验 | 已有 legacy SDK runner、示例、校验、审核包、PR/workflow 工具、developer-first local extension 文档边界，以及 `entries.commands` compatibility runtime 支持 | runtime 补齐 shell command execution、service/dashboard lifecycle、setup、health、bridge 与第三方提交演练 |
...
| Phase 56 | Extension Command Entries Runtime | P1 | 已完成 command entries runtime | `entries.commands` 可在带 `main` 的 JavaScript compatibility 包中通过现有 runner 执行；service/dashboard entries 仅作为声明暴露；manifest/install review/shared contract 测试补齐。 |
