...
| Phase 72 | Plugin Service Process-Tree Hardening | P1 | 已完成 service-only harder cleanup truth | 仅对 declaration-only service entries 增加宿主 descendant verification；root exit 后若已知 descendants 仍存活，则 stop 结果 fail-closed，不再宣称 clean stop；setup / command cleanup 保持现状。 |
