...
状态：Phase 1 已落地，当前 TODO 入口以 `docs/openpet-current-todo-architecture.md` 为准

## 目标

AI Talk 的目标是把 OpenPet 从“AI 配置 + 简单聊天”升级成可持续演进的宠物对话系统。系统需要支持当前激活 pet-pack 的独立人格、独立主会话、长期记忆、动作建议、可诊断日志，并且让 Control Center 和桌面聊天窗口共享同一套会话能力，为后续流式回复、多会话 UI、向量检索和插件扩展预留边界。
...
- 本阶段不继续扩展桌面聊天交互形态；基础入口已由 `PetChatWindowManager` 和 `src/main/pet-chat/` 落地。
...
9. `PetChatWindowManager` 管理桌面聊天窗口，并复用 AI Talk 状态。
10. `PluginService` 接收 `aiService` 与 `aiTalkService`，但 AI Talk 第三方扩展 API 仍未开放。
11. `registerIpcHandlers()` 注入 `aiTalkService`，AI IPC 优先走 talk 编排。
...
- memory profile 展示、单条删除、清空当前宠物关系记忆。
- AI 聊天。
- 打开桌面聊天窗口。
...
- Memory UI 已有基础列表、单条删除、清空当前宠物关系记忆，但还没有恢复、导出、评分解释或高级隐私策略。
...
### 2. AI Memory 管理 UI 已有基础，需要继续增强

当前已有 `memory.enabled` 开关、记忆列表、单条删除、清空当前宠物关系记忆和最近 memory job 展示。下一步不再是从零做管理 UI，而是让记忆更可解释、更可恢复、更适合排查问题。

后续可补齐：

- memory restore；
- memory export redacted diagnostics；
- memory score/reason display；
- filtered candidate count and trace drilldown；
- global memory clear with stronger confirmation。
...
状态：基础能力已完成。自动记忆现在可见、可删、可清空当前宠物关系记忆。

仍可继续增强：

- 恢复误删记忆；
- 导出脱敏 memory diagnostics；
- 展示记忆注入原因；
- 增加 global memory 清空确认。
