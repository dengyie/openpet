...
> **项目状态：v1.0 产品化基线完成；当前版本 v1.0.1-rc.2 覆盖 OpenPet 改名兼容、Codex pet 导入、内置 pet packs 与 TypeScript 迁移框架；macOS 分发基线已完成，Windows 打包/CI/签名策略/冒烟证据、报告、runbook 与 collector/证据包校验/summary/archive-manifest、desktop picker smoke evidence 工具基线已落地但尚未 release-ready**
...
| Node 原生 test runner | — | Service / release / IPC / example plugin / plugin submission / sandbox evaluation / RC upgrade smoke / Codex pet import / Codex pet zip import / bundled Codex pets / pet pack export / AI behavior diagnostics / TypeScript migration gate / desktop picker evidence / packaged runtime evidence 测试框架（382 个测试） |
| Playwright | ^1.60 | Control Center UI 回归测试（10 个测试） |
...
- ✅ Pet pack 运行时：manifest schema + loader + importer + catalog + Codex pet adapter
- ✅ Control Center：Vite + React，7 个 Tab（从 1364 行重构为 62 行），并有 Playwright UI 回归基线
- ✅ AI 集成：OpenAI-compatible，API Key 隔离，结构化行为编排、决策查看、replay 与脱敏诊断导出
...
**质量指标**：
- ✅ 382/382 Node 测试通过（service / release / 示例插件 / 插件提交校验、审核包、PR packet、workflow bundle 与 bundle validation、plugin sandbox evaluation、RC upgrade smoke、Codex pet import、Codex pet zip import、bundled Codex pets、pet pack export、AI behavior diagnostics、TypeScript migration gate 门禁覆盖）
...
│   └── validate-plugin-package.js # 插件包提交前校验 CLI
├── src/
│   ├── main/
...
│   ├── examples/                  # 示例插件 install/run 覆盖
│   ├── scripts/                   # CLI 工具测试
│   └── release/                   # Windows release evidence / report 工具测试
├── examples/
│   └── plugins/
│       ├── focus-timer/           # Focus Timer 本地插件示例（manifest/config/schema/entry/README）
│       ├── weather-status/        # Weather Status 本地插件示例（network allowlist/storage/pet speech）
│       └── rss-reader/            # RSS Reader 本地插件示例（public feed/network/storage/pet speech）
├── cat_anime/
...
npm test                      # 运行 Node 测试（382 个测试）
npm run test:control-center   # 运行 Control Center Playwright UI 回归测试（10 个测试）
