...
- Extension docs must be honest: OpenPet now parses declarations, can explicitly run `entries.setup` for enabled policy-allowed local plugins, can run `entries.commands` through the JavaScript compatibility runner when `main` exists, can explicitly run declaration-only local `entries.commands` as short-lived processes with JSON stdin context and a short-lived bridge, can explicitly open declared HTTP/HTTPS dashboards for enabled plugins, can explicitly start/stop declared local service entries, can give explicitly started services the same narrow bridge, can expose a read-only action catalog to bridge runs, can manually check declared loopback service health endpoints, and keeps service runtime in `stopping` until exit confirmation after best-effort process-group cleanup attempts. Command, setup, and service processes are spawned without shell expansion. Services do not auto-start, setup and command entries do not run during install or enable, health checks do not run in the background, and hard process-tree cleanup guarantees are not implemented yet; OpenPet does not claim complete sandboxing for arbitrary local processes.
...
2. Use Phase 64 plugin command bridge plus Phase 65 service cleanup semantics plus Phase 66 service bridge semantics plus Phase 67 action-catalog semantics as the current plugin runtime boundary: declaration-only command runs get a short-lived bridge URL/token for `pet.say`, `pet.action`, `pet.event`, read-only context, and read-only action discovery, while declared services now get the same narrow bridge during explicit service runs and stay `stopping` until exit confirmation after a stop request.
...
4. After Phase 67, start the next concrete phase from real evidence work, community extension rehearsal, harder descendant-process guarantees, richer but still narrow authoring bridges, or another high-drift service/report boundary.
