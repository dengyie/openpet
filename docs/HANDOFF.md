...
3. [`development-summary.md`](./development-summary.md)：截至 Phase 32 的阶段开发小结、质量基线和下一步建议。
...
5. 最新的 `docs/phases/phase-*.md` 与 `docs/reviews/phase-*-review.md`：具体阶段的实现记录、review、验证和残留风险。当前最新阶段为 [`phase-32-bundled-codex-pets.md`](./phases/phase-32-bundled-codex-pets.md) 与 [`phase-32-bundled-codex-pets-review.md`](./reviews/phase-32-bundled-codex-pets-review.md)。
...
|------|------|------|
| **功能完整性** | 95% | 所有承诺功能已实现 |
| **测试覆盖** | 317 Node + 9 UI ✅ | service / release / 主进程 IPC / 示例插件 / 插件提交校验、审核包、PR packet、workflow bundle 与 workflow bundle validation / RC upgrade smoke evidence / Codex pet import / Codex pet zip import / bundled Codex pets / desktop picker smoke evidence 门禁覆盖；Control Center Playwright UI 回归基线 |
| **架构质量** | ⭐⭐⭐⭐⭐ | 分层清晰、安全可靠 |
| **代码质量** | ⭐⭐⭐⭐⭐ | 模块化彻底、职责单一 |
...
```bash
npm test                  # 317 Node tests, all pass
...
docs/development-summary.md                # 截至 Phase 32 的阶段开发小结、质量基线与下一步建议
