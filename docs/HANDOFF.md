...
- Image generation for Creator Studio is host-owned at the provider boundary. Control Center now exposes one OpenAI-compatible image Provider surface with presets, optional `/models` discovery, compatibility hints, and health checks; `ImageGenerationModelService` owns provider calls and output writes, and Creator Studio consumes host-mediated outputs instead of receiving provider credentials.
- Creator Studio's dashboard-first wizard is now the primary user-facing workflow for run draft, follow-up questions, confirmation, generation, QA review, retry/recover, prompt provenance, workflow guidance, and import handoff. Approved action/pet runs now surface a structured handoff card that tells the dashboard which Control Center plugin command to run next, while import itself remains blocked to explicit command runs so bridge tokens stay command-scoped. The command entrypoints remain the automation and host-bridge entry path.
