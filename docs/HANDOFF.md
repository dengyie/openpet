...
38. Continue Creator Studio from the current split: plugin owns prompts, `GenerationTask`, run persistence, QA, preview, review, and dashboard guidance; OpenPet host owns model secrets, provider calls, output writes, pet/action import, and final trigger persistence.
39. Continue trigger work by adding a host trigger-rule schema/editor for `random`, `state`, and `event` proposals; do not let plugins silently mutate those bindings before host review exists.
40. Continue release/productization from collecting real signed workflow artifacts, receiving a compatible live community source that passes discovery, intake, bridge, and Phase 99 evidence, or another high-drift service/report boundary.
