...
| `plugin-service.js` | 插件发现、启用/禁用、命令运行、内存运行日志 |
...
```
src/main/services/plugin-service.js      # 插件运行日志、错误记录、清空日志
src/control-center/src/main.jsx          # Plugins 页运行日志/错误面板
src/control-center/src/styles.css        # 插件日志面板样式
src/main/ipc.js                          # 插件日志 IPC handler
control-center-preload.js                # 暴露插件日志 API
tests/services/plugin-service.test.js    # 插件日志回归测试
```
...
- [ ] Control Center main.jsx（约 1182 行）需要拆分为多个组件
