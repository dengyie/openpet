...
> 当前状态：v1.0 产品化基线已完成；v1.0.1-rc.1 完成 OpenPet 改名、GitHub 仓库迁移与升级兼容；macOS 分发基线已完成，Windows 桌面分发处于设计阶段
...
- 📝 Windows 桌面分发设计已记录在 [`desktop-release-design.md`](./desktop-release-design.md)，后续需补 build config、CI、签名策略和冒烟验证
...
| **可发布性** | ✅ macOS RC 可发布 | Windows 桌面分发尚未 release-ready |
...
- [ ] Windows 桌面分发：`build/win`、`build/icon.ico`、`windows-latest` release job、签名策略、安装/卸载/透明窗口冒烟验证
