...
```
catalog/ibot-catalog.json                # 内置生态 catalog 元数据
...
- [x] **插件安装与权限 review**：
  - 新增 `PluginInstallService`，支持插件目录 / `.ibot-plugin.zip` 检查、安装、更新、卸载。
...
- [x] **AI 行为编排**：
  ...
  - Phase 4 已新增结构化 `ibot_behavior` tool-call intent、可配置规则、actionId 白名单、cooldown、dry-run 和最近决策日志
