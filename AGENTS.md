...
- **Control Center** (`src/control-center/`): React + Vite web app embedded in Electron BrowserWindow, with Pet / Actions / AI / Plugins / Catalog / Service / About tabs
...
- Use `npm run test:core` for core runtime Node regressions
- Use `npm run test:core:all` before merging user-facing runtime changes
- Use `npm run test:tools` for release, evidence, scaffold, and maintenance tooling tests
- Use `npm test` to run all Node tests using Node native test runner
...
- Run `npm run test:core` for core runtime work, `npm run test:tools` for tooling work, and `npm test` for full Node verification
