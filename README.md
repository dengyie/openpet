...
- Developer-first local extension model with current legacy SDK compatibility, explicit command/dashboard/service controls, narrow pet bridge access for explicit commands/services including read-only action discovery and bounded action preset updates, validation, logs, catalog install, and uninstall flow.
...
OpenPet uses one third-party package model: an extension. The package manifest is still named `plugin.json` for compatibility. The host now normalizes and inspects extension declarations for `entries.setup`, `entries.commands`, `entries.services`, `entries.dashboards`, `manifest`, `config`, and `assets`; JavaScript compatibility packages can expose `entries.commands` through the existing runner, and declaration-only local extensions can run short-lived `entries.commands` as explicit user actions with JSON stdin context and a short-lived bridge for `pet.say`, `pet.action`, `pet.event`, bounded context reads, read-only action discovery, and bounded action preset updates for existing action ids. Enabled plugins can explicitly run declared setup entries, open declared HTTP/HTTPS dashboards, start or stop declared service entries, give explicitly started service processes the same narrow bridge while they are running, and manually check declared loopback service health endpoints from Control Center. The preset bridge updates only `defaultAction` and `clickAction` through the host save path; it does not expose sprite editing, asset import, atlas data, or arbitrary file writes. Command, setup, and service processes are spawned without shell expansion, services do not auto-start, setup and commands do not run during install or enable, service stops attempt best-effort process-group cleanup, and health checks do not run in the background. Arbitrary shell consoles and hard process-tree guarantees remain future runtime work.
