...
const { BUILT_IN_PACKS, REQUIRED_CHECKS } = require('./validate-packaged-runtime-smoke-report')

const DEFAULT_RELEASE_DIR = path.join(__dirname, '..', 'release')
const DEFAULT_OUTPUT_PATH = path.join(DEFAULT_RELEASE_DIR, 'packaged-runtime-smoke-report.json')

const usage = () => [
  'Usage: node scripts/create-packaged-runtime-smoke-report.js [--platform <darwin|win32>] [--release-dir <dir>] [--output <report.json>] [--allow-any-platform]',
  '',
  'Creates a pending packaged OpenPet runtime smoke report.',
  'The report is a structure and evidence container only; real runtime checks remain pending until a packaged-app smoke run fills them.'
].join('\n')
...
const createPendingChecks = () => REQUIRED_CHECKS.map((check) => ({
  ...
  status: 'pending',
  evidence: '',
  notes: `${check.label}. Fill with evidence from a real packaged OpenPet runtime smoke validation run.`
}))
...
const pickArtifacts = ({ releaseDir, platform, files, fsImpl = fs }) => {
  if (platform === 'darwin') {
    ...
    const zip = files.find((fileName) => /\.zip$/i.test(fileName) && !hasWindowsToken(fileName)) || ''
  ...
  if (platform === 'win32') {
    ...
    const zip = files.find((fileName) => /\.zip$/i.test(fileName) && !hasMacToken(fileName)) || ''
  ...
  throw new Error(`Unsupported platform for packaged runtime smoke report: ${platform}`)
}
...
const defaultBuiltInPackFixtures = () => Object.fromEntries(BUILT_IN_PACKS.map((packId) => [
  packId,
  packId === 'legacy-cat' ? 'cat_anime/' : `assets/pet-packs/${packId}/`
]))

const createPackagedRuntimeSmokeReport = ({
  ...
  if (!['darwin', 'win32'].includes(platform)) throw new Error('Packaged runtime smoke reports only support darwin and win32')
  if (!allowAnyPlatform && platform !== process.platform) {
    throw new Error('Packaged runtime smoke reports must be generated on their target platform unless --allow-any-platform is used for structure checks')
  }
  ...
  return {
    ...
    fixtures: {
      builtInPacks: defaultBuiltInPackFixtures(),
...
const main = () => {
  ...
  const report = createPackagedRuntimeSmokeReport(options)
  const outputPath = writeReport({ report, outputPath: options.outputPath })

  console.log(`Packaged runtime smoke report created: ${outputPath}`)
  console.log(`Platform: ${report.platform}`)
  console.log(`Artifact: ${report.artifact.appPath || report.artifact.installer || report.artifact.zip || 'not discovered'}`)
  console.log('Runtime smoke checks remain pending until a packaged-app validation run fills evidence.')
}
